...
L.Control.Details = L.Control.extend({
	onAdd: map => {
		...
		details.classList.add('details_container');
...
const Leaflet = {
	...
	getLocalityColumnsFromSelectedCell(locality_columns, selected_column) {
		...
		if (locality_columns.length > 1) {
			// if multiple localities present in a row, check, which group this field belongs too
	...
	addDetailsButton(container, map, details) {
		...
		const details_container = container.getElementsByClassName('details_container')[0];
		details_container.getElementsByTagName('span').innerHTML = details;
	...
	showCOMap(map_container, list_of_layers_raw, details = undefined) {
				...
				(
					{
						...
						tile_layer: leaflet_tile_servers[(
							transparent ? 'overlays' : 'base_maps'
						)][layer_label],
					}
				),
			),
		...
		if (typeof details !== 'undefined')
			return [map, layer_group, this.addDetailsButton(map_container, map, details)];
